...
app.get("/todos/", async (request, response) => {
    ...
    switch (true) {
        ...
        case hasStatusProperty(request.query):
            if (status === "TO DO" || status === "IN PROGRESS" || status === "DONE") {
                ...
                response.status(400);
                response.send("Inavalid Todo Status");
            }
            break;
    ...
    case hasSearchProperty(request.query):
      ...
      data = await db.all(getTodosQuery);
      response.send(data.map((eachItem)=>
        convertDataIntoResponseObject(eachItem))
      );
    ...
    case hasCategoryAndPriorityProperties(request.query):
      if (
        ...
        if(
            ...
            response.status(400);
            respo.send("Invalid todo Property")
        }
      }else{
        response.status(400);
        respo.send("Invalid todo Category")
      }
      break;
